...
// Once Alpes OS produces events (lib/sbqueue.js), index links and log entries are
// the curator's job: the two helpers below then do nothing.
...
export function linkInIndex(dir, slug, title, desc) {
  ...
  if (!bundle.exists(file)) return;
  const txt = bundle.read(file);
  ...
  bundle.replace(file, lines.join('\n') + '\n');
}
...
export function linkUnderHeading(file, heading, bullet, why = '') {
  if (!bundle.exists(file)) return;
  let txt = bundle.read(file).replace(/\s+$/, '');
  ...
  bundle.replace(file, txt + '\n', { why: why || `ajout de « ${bullet} » sous « ${heading} »`, subject: `Lien ${path.basename(file)}` });
}
