...
export function makeChantiers(root, cfg = {}, hooks = {}) {
  ...
  const view = c => ({ ...c, enCours: live.has(c.id), dossier: path.join(dir, c.statut === 'clos' ? c.repo : c.worktree) });
  ...
  function close(id) {
    ...
    if (fs.existsSync(worktree)) {
      ...
      try {
        ...
        // a half-done merge would block every later close on this project
        spawnSync('git', ['merge', '--abort'], { cwd: repo });
        throw new Error('conflit avec main : relance le chantier pour qu\'il intègre main, puis clôture-le');
      }
