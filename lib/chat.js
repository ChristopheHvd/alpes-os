...
const DECISION_MS = Number(process.env.ALPES_OS_DECISION_MS) || 2 * 3600e3;  // how long a write tool waits on a click — a chat
// isn't Slack-instant; the user may be away from the screen for a while
...
Un projet a presque toujours un client. Avant de créer un projet, vérifie si le client existe
(lister_clients, ou une recherche). S'il manque, propose d'abord creer_client, puis crée le
projet en passant le slug de la fiche au paramètre client — les deux fiches se relient alors
l'une à l'autre. Ne crée pas de client sans projet ni raison claire.
...
Tu ne produis donc rien toi-même : quand il faut fabriquer quelque chose (maquettes, code,
document, analyse longue), tu le délègues avec deleguer_chantier. Le chantier ne voit pas cette
conversation : son objectif doit être autoportant — contexte trouvé dans le second brain,
décisions déjà prises, contraintes, livrables attendus, critère de fin. Un chantier est rattaché
à un projet : s'il n'a pas de fiche, propose d'abord creer_projet.

Quand un message commence par [Chantier …], c'est un chantier qui revient : résume en trois
lignes ce qu'il a produit (chemins des livrables), relaie sa question s'il en pose une, et
propose la suite (retouche via relancer_chantier, ou clôture si Christophe valide).`;

// The process dies after IDLE_MS or with the server; the CLI's session file does
// not. Each conversation id maps to a fixed session UUID, so a respawn resumes it
// with the whole exchange instead of starting blank.
function sessionOf(id) {
  const h = createHash('sha1').update(`alpes-os-chat:${id}`).digest('hex');
...
export function makeChat(root, cfg) {
  ...
  function ensure(id) {
    ...
    args.push(...(sessionExists(sid) ? ['--resume', sid] : ['--session-id', sid]));

    const child = spawn(cfg.bin, args, { cwd: root, env: { ...process.env, CLAUDECODE: '', MCP_TOOL_TIMEOUT: String(DECISION_MS + 60e3) }, stdio: ['pipe', 'pipe', 'pipe'] });
    c = { id, child, buf: '', listeners: new Set(), pending: new Map(), touched: Date.now(), busy: false, turn: null };
    ...
    child.on('close', () => {
      emit(c, { type: 'closed' });
      // a killed process can take seconds to exit; by then the same id may
      // already run a fresh one, which must not be dropped
      if (conversations.get(id) === c) conversations.delete(id);
    });
  ...
  function route(c, ev) {
    if (ev.type === 'stream_event') {
      ...
      if (d?.type === 'content_block_delta' && d.delta?.type === 'text_delta') {
        if (c.turn) c.turn.assistant += d.delta.text;
        emit(c, { type: 'text', text: d.delta.text });
      }
      return;
    }
    ...
    if (ev.type === 'result') {
      c.busy = false;
      // The whole turn is journalled at once, at the end: a process killed mid
      // stream then leaves no half-written exchange in the second brain.
      flush(c);
      emit(c, { type: 'done', error: ev.is_error ? (ev.result ?? c.lastError ?? 'échec') : undefined });
      if (c.inbox?.length) send(c.id, c.inbox.shift(), true);
    }
  }
  ...
  function flush(c) {
    ...
    c.turn = null;
    if (!t) return;
    try { cfg.log?.append(c.id, t); } catch (e) { /* the journal must never break the chat */ }
  }
  const record = (c, summary, decided) => { if (c.turn) c.turn.actions.push({ summary, decided }); };

  // `injected` marks a message the dashboard wrote on the user's behalf (a
  // chantier coming back): the page shows it as a notice, not as their words.
  function send(id, text, injected = false) {
    const c = ensure(id);
    flush(c);   // a turn that never reached `result` is still worth keeping
    c.turn = { user: String(text), assistant: '', actions: [] };
    c.busy = true;
    if (injected) emit(c, { type: 'injected', text: String(text) });
  ...
  return {
    ...
    // Hand the agent news it didn't ask for, e.g. a delegated chantier coming
    // back. Mid-turn, it waits for the turn to end rather than cut into it; a
    // conversation whose process was released is respawned on its session.
    inject(id, text) {
      const c = conversations.get(id);
      if (c?.busy) { (c.inbox ??= []).push(String(text)); return { ok: true, queued: true }; }
      return send(id, text, true);
    },
    ...
    propose(id, action) {
      ...
      return new Promise(resolve => {
        const timer = setTimeout(() => {
          ...
          record(c, action.summary, 'expiré');
    ...
    decide(id, key, approved) {
      ...
      c.pending.delete(key);
      emit(c, { type: 'confirm-gone', key });
      record(c, p.card.summary, approved ? 'validé' : 'refusé');
    ...
    sweep() {
      for (const [id, c] of conversations) if (Date.now() - c.touched > IDLE_MS && !c.busy) { flush(c); c.child.kill(); conversations.delete(id); }
    },
  };
}
