...
export function makeChatLog(secondBrain) {
  ...
  const readRaw = () => (bundle.exists(file) ? bundle.read(file) : HEADER);
  ...
  function append(id, { user = '', assistant = '', actions = [] }) {
    ...
    // A conversation keeps one section, so a later turn goes at the end of that
    // section — not at the end of the file, where another conversation may have
    // opened its own section in the meantime.
    const heading = readRaw().split('\n').find(l => new RegExp(`^##\\s.*·\\s+conversation\\s+${id}\\s*$`).test(l));
    bundle.append(file, turn, {
      section: heading || `## ${stamp()} · conversation ${id}`, header: HEADER,
      subject: `Conversation ${id}`, why: `échange avec l'assistant d'Alpes OS (conversation ${id})`,
    });
    return { id, file };
  }
