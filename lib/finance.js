...
export function listFinance(root) {
  ...
  return fs.readdirSync(dir).filter(f => f.endsWith('.md') && f !== 'index.md').map(f => read(path.join(dir, f), root));
}

function rewrite(root, slug, change) {
  const abs = path.join(root, 'brain', 'finance', `${slug}.md`);
  if (!/^[\w.-]+$/.test(slug) || !fs.existsSync(abs)) throw new Error('note finance introuvable');
...
export function addEcheance(root, slug, e) {
  ...
  const note = rewrite(root, slug, list => [...list, item]);
