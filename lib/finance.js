...
// sources: is a block-style YAML list ("- id: …\n  resource: …\n  title: …"),
// unlike echeances' inline { } style — parsed line by line for that shape.
function parseSources(fm) {
  ...
  for (let i = start + 1; i < lines.length; i++) {
    const l = lines[i];
    if (!/^[ \t]/.test(l)) break;
    const item = l.match(/^[ \t]+-\s+(\w+):\s*(.*)$/);
    if (item) { cur = { id: '', resource: '', title: '' }; out.push(cur); cur[item[1]] = unquote(item[2]); continue; }
    const field = l.match(/^[ \t]+(\w+):\s*(.*)$/);
