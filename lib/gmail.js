...
const SCOPES = [
  ...
  // write access is what lets the chat agent turn "RDV mardi 14h" into a real event
...
export function makeGmail(credDir, port, opts = {}) {
  ...
  const status = () => ({
    ...
    hasCalendarWrite: storedScopes().includes('https://www.googleapis.com/auth/calendar.events'),
  });
  ...
  return {
    ...
    async flagged({ label, maxThreads, newerThanDays, query = '', filter = {} }) {
      ...
      const threads = await Promise.all((list.threads ?? []).map(async t => {
        ...
        return {
          bucket: c.bucket, why: c.reasons, known: c.isKnown, addr,
          id: th.id,
      ...
      const isHidden = filter.isHidden ?? (() => false);
      const visible = threads.filter(t => !isHidden(t.id));
      const hiddenCount = threads.length - visible.length;
      const kept = visible.filter(t => t.bucket === 'personne').slice(0, maxThreads);
      const others = visible.filter(t => t.bucket !== 'personne').slice(0, 20);
      return {
        label, query: q, syncedAt: new Date().toISOString(),
        inbox: (all.threads ?? []).length, unread: (unreadList.threads ?? []).length,
        counts, hiddenCount, threads: kept, others,
