...
export function makeGmail(credDir, port) {
  ...
  return {
    ...
    async flagged({ label, maxThreads, newerThanDays, query = '' }) {
      ...
      const [all, unreadList, list] = await Promise.all([
        ...
        api('/threads', { labelIds: lab.id, maxResults: maxThreads, q }),
      ]);
      ...
      return { label, query: q, total: (all.threads ?? []).length, unread: (unreadList.threads ?? []).length, threads, syncedAt: new Date().toISOString() };
    },
  };
}
