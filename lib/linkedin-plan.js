...
// the morning pass: today's post (after sliding), not prepared yet, past the hour set in
// config, nothing already running for it and fewer than two tries today
export function dueForPreparation({ post, now = new Date(), prepareAt = '06:00', running = false, attempts = 0 }) {
  if (!post || post.statut !== 'a_produire' || running || attempts >= 2) return false;
  const hhmm = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
  return hhmm >= prepareAt;
}

// what a preparation must produce for this post: always the text, the visual only when
// Claude can make it and it isn't there yet
export const toProduce = post => ['texte', ...(post.visualKind === 'auto' && !post.visual ? ['visuel'] : [])];

// the plan's "## Cadre" and "## Production des visuels" sections, handed to the skill as is
export function planFrame(txt) {
  return txt.match(/^## Cadre\n[\s\S]*?(?=^## Calendrier)/m)?.[0].trim() ?? '';
}

// the skill's output file: "## Texte" then "## Premier commentaire"
export function parsePrepared(md) {
  const part = h => md.match(new RegExp(`^## ${h}\\s*\\n([\\s\\S]*?)(?=^## |(?![\\s\\S]))`, 'm'))?.[1].trim() ?? '';
  return { texte: part('Texte'), commentaire: part('Premier commentaire') };
}
