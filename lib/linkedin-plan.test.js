...
import { parsePlan, schedule, loadLinkedin, updatePost, visualKind, addDays, dueForPreparation, toProduce, parsePrepared, planFrame } from './linkedin-plan.js';
...
test('morning pass: only after the hour, once, never twice in parallel', () => {
  const post = { statut: 'a_produire' };
  const at = h => new Date(`2026-10-01T${h}:00`);
  assert.equal(dueForPreparation({ post, now: at('05:50'), prepareAt: '06:00' }), false);
  assert.equal(dueForPreparation({ post, now: at('06:00'), prepareAt: '06:00' }), true);
  assert.equal(dueForPreparation({ post, now: at('09:40'), prepareAt: '06:00' }), true);   // Mac woke up late
  assert.equal(dueForPreparation({ post, now: at('07:00'), running: true }), false);
  assert.equal(dueForPreparation({ post, now: at('07:00'), attempts: 2 }), false);
  assert.equal(dueForPreparation({ post: { statut: 'prepare' }, now: at('07:00') }), false);  // slid from yesterday, already done
  assert.equal(dueForPreparation({ post: null }), false);
});

test('what to produce, and reading the skill output back', () => {
  assert.deepEqual(toProduce({ visualKind: 'auto', visual: null }), ['texte', 'visuel']);
  assert.deepEqual(toProduce({ visualKind: 'photo', visual: null }), ['texte']);
  assert.deepEqual(parsePrepared('## Texte\n\n🧠 Ligne 1\n\nLigne 2\n\n## Premier commentaire\n'), { texte: '🧠 Ligne 1\n\nLigne 2', commentaire: '' });
  assert.match(planFrame('# P\n\n## Cadre\n\n- a\n\n## Production\n\nx\n\n## Calendrier\n'), /^## Cadre[\s\S]*## Production[\s\S]*x$/);
});
