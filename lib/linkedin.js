...
export function makeLinkedin(credDir, { port, version = '202609' } = {}) {
  ...
  // text + optional visual. A failure after the post request left without a clear
  // answer is flagged `ambiguous`: never retried blindly. No first comment here: the
  // comments API needs Community Management (partners only), w_member_social gets a 403.
  async function publish({ text, file = null, title = '' }) {
