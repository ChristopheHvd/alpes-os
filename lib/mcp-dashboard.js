...
// Plain node:http rather than fetch: fetch gives up on a response that takes
// more than 5 minutes, and this one waits for as long as the card stays up.
function waitPost(p, body) {
  return new Promise((resolve, reject) => {
    const req = request(API + p, { method: 'POST', headers: { 'content-type': 'application/json' } }, res => {
      let text = '';
      res.setEncoding('utf8');
      res.on('data', d => { text += d; });
      res.on('end', () => {
        let data; try { data = JSON.parse(text); } catch { data = { raw: text.slice(0, 500) }; }
        if (res.statusCode >= 400) reject(new Error(data.error ?? `${res.statusCode} sur ${p}`));
        else resolve(data);
      });
    });
    req.on('error', reject);
...
async function confirm(name, summary, details) {
  const v = await waitPost('/api/chat/propose', { id: CHAT, action: { name, summary, details } });
...
const TOOLS = [
  ...
  // ---- chantiers: work delegated to a background Claude Code run ----
  {
    name: 'deleguer_chantier',
    description: "Délègue un travail à produire (maquettes, code, document, analyse longue) à un Claude Code autonome en tâche de fond, dans le repo du projet. Demande validation. L'objectif doit être autoportant : le chantier ne voit pas cette conversation — mets-y le contexte trouvé dans le second brain, les contraintes, les livrables attendus et quand c'est fini. Tu seras prévenu ici quand il revient.",
    inputSchema: {
      type: 'object',
      properties: {
        titre: str,
        objectif: { ...str, description: 'consigne complète et autoportante' },
        projet: { ...str, description: 'slug de la fiche projet (lister_projets) — un repo par projet' },
      },
      required: ['titre', 'objectif', 'projet'],
    },
    run: async a => {
      const decided = await confirm('deleguer_chantier', parts(
        `Déléguer le chantier « ${a.titre} »`,
        `projet ${a.projet}`,
        String(a.objectif ?? '').replace(/\s+/g, ' ').slice(0, 160),
      ), a);
      const bail = declined(decided);
      if (bail) return bail;
      const c = await post('/api/chantiers', { titre: a.titre, objectif: a.objectif, projet: a.projet, chatId: CHAT });
      return { fait: true, chantier: c.id, statut: c.statut, dossier: c.dossier, note: 'il tourne en tâche de fond ; son retour arrivera dans cette conversation' };
    },
  },
  {
    name: 'suivre_chantiers',
    description: "État des chantiers délégués : statut (en_attente, en_cours, pret, bloque, echec, interrompu, clos), activité en cours, résumé, question posée, livrables. Avec id : le détail et la fin du journal du run. Les livrables se lisent avec Read, dans `dossier`.",
    inputSchema: { type: 'object', properties: { id: str } },
    run: async a => {
      if (a.id) {
        const c = await get('/api/chantiers/' + encodeURIComponent(a.id));
        return { ...c, log: String(c.log ?? '').slice(-2000) };
      }
      const all = await get('/api/chantiers');
      return all.slice(0, 20).map(c => ({
        id: c.id, titre: c.titre, projet: c.projet, statut: c.statut, activite: c.activite, raison: c.raison,
        resume: c.resume, question: c.question, livrables: c.livrables, dossier: c.dossier,
        etapes: c.etapes.length, prochaineReprise: c.prochaineReprise,
      }));
    },
  },
  {
    name: 'relancer_chantier',
    description: "Relance un chantier (prêt, bloqué, en échec ou interrompu) avec une nouvelle consigne : une retouche, ou la réponse à sa question. Il reprend la même session et se souvient de tout. Demande validation.",
    inputSchema: { type: 'object', properties: { id: str, consigne: str }, required: ['id', 'consigne'] },
    run: async a => {
      const decided = await confirm('relancer_chantier', parts(`Relancer le chantier ${a.id}`, String(a.consigne ?? '').replace(/\s+/g, ' ').slice(0, 160)), a);
      const bail = declined(decided);
      if (bail) return bail;
      const c = await post(`/api/chantiers/${encodeURIComponent(a.id)}/resume`, { consigne: a.consigne });
      return { fait: true, chantier: c.id, statut: c.statut };
    },
  },
  {
    name: 'arreter_chantier',
    description: "Arrête un chantier en cours ou en attente. Il pourra être relancé plus tard. Demande validation.",
    inputSchema: { type: 'object', properties: { id: str }, required: ['id'] },
    run: async a => {
      const bail = declined(await confirm('arreter_chantier', `Arrêter le chantier ${a.id}`, a));
      if (bail) return bail;
      const c = await post(`/api/chantiers/${encodeURIComponent(a.id)}/stop`, {});
      return { fait: true, chantier: c.id, statut: c.statut };
    },
  },
  {
    name: 'cloturer_chantier',
    description: "Clôture un chantier terminé et validé par Christophe : sa branche est fusionnée dans main du repo du projet, son worktree supprimé. À ne proposer que quand il a dit que le résultat lui convient. Demande validation.",
    inputSchema: { type: 'object', properties: { id: str }, required: ['id'] },
    run: async a => {
      const bail = declined(await confirm('cloturer_chantier', `Clôturer le chantier ${a.id} et fusionner son travail dans le repo du projet`, a));
      if (bail) return bail;
      const c = await post(`/api/chantiers/${encodeURIComponent(a.id)}/close`, {});
      return { fait: true, chantier: c.id, statut: c.statut, livrables: c.livrables, dossier: c.dossier };
    },
  },
];
