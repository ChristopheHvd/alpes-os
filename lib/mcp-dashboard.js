...
const TOOLS = [
  ...
  // ---- writes: proposed, then confirmed by the user ----
  {
    ...
    run: async a => {
      const decided = await confirm('creer_projet', `Créer le projet « ${a.titre} »`, a);
  ...
  },
  {
    ...
    run: async a => {
      const decided = await confirm('maj_projet', `Mettre à jour le projet ${a.slug}`, a);
      const bail = declined(decided);
      if (bail) return bail;
  ...
  },
  {
    ...
    run: async a => {
      const decided = await confirm('ajouter_tache', `Ajouter « ${a.texte} » à aujourd'hui`, a);
      const bail = declined(decided);
      if (bail) return bail;
  ...
  },
  {
    ...
    run: async a => {
      const decided = await confirm('creer_evenement', `Créer « ${a.titre} » le ${a.debut}`, a);
      const bail = declined(decided);
      if (bail) return bail;
  ...
  },
  {
    ...
    run: async a => {
      const decided = await confirm('lancer_micro_app', `Lancer ${a.app} · ${a.action}`, a);
      const bail = declined(decided);
      if (bail) return bail;
