...
import { clientResources, readManual } from './resources.js';

const FM = /^---\n[\s\S]*?\n---/;
const norm = s => String(s ?? '').normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase();
const body = (root, id) => bundle.read(path.join(root, id)).replace(FM, '').trim();
...
export function portfolio(root, { todo = { items: [], carried: [], later: [] }, events = [], alpesRoot = '' } = {}) {
  const manual = alpesRoot ? readManual(alpesRoot) : {};
  ...
  return {
    clients: clients.map(c => {
      ...
      return {
        ...
        resources: alpesRoot ? clientResources({ root, alpesRoot, client: c, notes: [c.body, ...own.map(p => p.body)], manual }) : [],
