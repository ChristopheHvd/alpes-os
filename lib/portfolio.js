...
const links = (txt, kind) => [...txt.matchAll(new RegExp(`\\]\\((?:\\.\\./)?${kind}/([\\w.-]+)\\.md\\)`, 'g'))].map(m => m[1]).filter(s => s !== 'index');
...
function listFinance(root) {
  const dir = path.join(root, 'brain', 'finance');
  if (!fs.existsSync(dir)) return [];
  return fs.readdirSync(dir).filter(f => f.endsWith('.md') && f !== 'index.md').map(f => {
    const txt = fs.readFileSync(path.join(dir, f), 'utf8');
    const fm = txt.match(FM)?.[0] ?? '';
    return {
      id: `brain/finance/${f}`, slug: f.slice(0, -3),
      title: field(fm, 'title') || f.slice(0, -3), type: field(fm, 'type'), description: field(fm, 'description'),
...
export function portfolio(root, { todo = { items: [], carried: [], later: [] }, events = [] } = {}) {
  ...
  const related = (names, extra = () => false) => {
    ...
    return {
      ...
      finance: finance.filter(extra).map(({ projects, clients, ...f }) => f),
    };
  };
  ...
  const projects = listProjects(root).map(p => {
    ...
    return { ...p, client, body: txt, ...related([p.title], f => f.projects.includes(p.slug)) };
  });
  ...
  return {
    clients: clients.map(c => {
      ...
      return {
        ...
        ...related(names, f => f.clients.includes(c.slug) || f.projects.some(s => own.some(p => p.slug === s))),
      };
    }),
    projects,
  };
