...
export function updateProject(secondBrain, slug, patch) {
  ...
  if (!path.resolve(abs).startsWith(path.resolve(secondBrain)) || !bundle.exists(abs)) return null;
  ...
  if (patch.client !== undefined) set('client', slugify(patch.client));
  set('updated', new Date().toISOString().slice(0, 10));
  txt = txt.replace(FM, `---\n${fm}\n---`);
  const why = [
    patch.stage && STAGES.includes(patch.stage) && `stade → ${patch.stage}`,
    patch.next !== undefined && `prochaine étape → « ${String(patch.next).trim()} »`,
    patch.client !== undefined && (patch.client ? `rattaché au client ${slugify(patch.client)}` : 'détaché de son client'),
  ].filter(Boolean).join(' ; ');
  bundle.replace(abs, txt, { why: `${slug} : ${why || 'revu'} (updated mis à jour)`, subject: `Projet ${slug}` });
...
export function createProject(secondBrain, { title, description = '', stage = 'actif', next = '', tags = [], body = '', client = '' }) {
  ...
  if (bundle.exists(abs)) throw new Error(`le projet ${slug} existe déjà`);

  const clientSlug = client ? slugify(client) : '';
  const clientFile = clientSlug ? path.join(secondBrain, 'brain', 'clients', `${clientSlug}.md`) : '';
  const clientOk = !!clientFile && bundle.exists(clientFile);
  const clientTitle = clientOk
    ? (bundle.read(clientFile).match(/^title:\s*(.+)$/m)?.[1].trim() || clientSlug)
    : '';
...
${next ? `- ${next}` : '- À définir'}
`;
  bundle.replace(abs, doc, { why: `nouvelle fiche projet « ${clean} »${clientOk ? ` pour le client ${clientSlug}` : ''} ; la lier depuis brain/projects/index.md et consigner l'ajout dans log.md`, subject: `Nouveau projet ${clean}` });
  // Wiring it in is best-effort — a missing index or log shouldn't sink the creation.
