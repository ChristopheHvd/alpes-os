...
export function makeRunner(root, cfg) {
  ...
  function start({ app, action, brief, model, from }) {
    ...
    const args = ['-p', prompt, '--model', model || app.model || cfg.defaultModel, '--permission-mode', cfg.permissionMode, '--allowedTools', cfg.allowedTools, '--add-dir', cfg.secondBrain, ...(app.addDirs ?? []).filter(Boolean).flatMap(d => ['--add-dir', d])];
    const run = {
