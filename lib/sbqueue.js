...
// the curator's queue (brain/inbox, untracked), and until the curator has archived that event the change is
...
${e.op === 'append' && e.header ? `\n## En-tête si le fichier est créé\n${fence(e.header)}markdown\n${e.header.replace(/\n$/, '')}\n${fence(e.header)}\n` : ''}`;
  ...
  // the queue root is the drop box; processing/ and archive/ sit inside it
  const dir = d => (d === 'inbox' ? conf.queueDir : path.join(conf.queueDir, d));
  ...
  const where = file => ['inbox', 'processing', 'archive'].find(d => fs.existsSync(path.join(dir(d), file)));
  ...
  // any dropped file counts, not only events: the curator files raw documents into Drive
  const count = d => { try { return fs.readdirSync(dir(d), { withFileTypes: true }).filter(f => f.isFile() && !f.name.startsWith('.')).length; } catch { return 0; } };
  ...
  return {
    ...
    isPending: abs => inBundle(abs) && entriesFor(rel(abs)).length > 0,
