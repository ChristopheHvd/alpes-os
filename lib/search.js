...
const FM = /^---\n([\s\S]*?)\n---/;
// the curator's working folders inside the queue are not knowledge
const QUEUE_DIRS = /brain\/inbox\/(processing|archive|curator\.lock)$/;
...
function walk(dir, out = []) {
  ...
  for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (ent.isDirectory()) { if (!QUEUE_DIRS.test(p)) walk(p, out); }
