...
export function makeStandup(secondBrain) {
  ...
  function readRaw() {
    return bundle.exists(file) ? bundle.read(file) : HEADER;
  }
  ...
  function append({ date = today(), slot = slotNow(), questions = [], answers = {}, outcome = '' }) {
    ...
    bundle.append(file, parts.join('\n').replace(/\n{3,}/g, '\n\n') + '\n', {
      header: HEADER, subject: `Stand-up ${date} ${slot}`,
      why: outcome ? `compte rendu du point du ${date} (${slot})` : `questions et réponses du point du ${date} (${slot})`,
    });
