...
const load = file => bundle.exists(file) ? parse(bundle.read(file)) : { days: {}, later: [] };
const save = (file, doc, why) => bundle.replace(file, serialize(doc), { why, subject: 'Todo du jour' });
...
export function writeTodo(file, items, later) {
  ...
  save(file, doc, 'todo modifiée depuis le dashboard (cases, ajouts, suppressions ou ordre)');
...
export function completeCarried(file, from, text, mark = 'x') {
  ...
  save(file, doc, `tâche du ${from} « ${hit.t} » passée à [${mark}]`);
...
export function tidyTodo(file) {
  ...
  const bumped = cap(doc);
  save(file, doc, 'rangement automatique après le point : reports repris, doublons exacts fusionnés, 5 tâches ouvertes au plus');
  return { ...readTodo(file), bumped };
}
