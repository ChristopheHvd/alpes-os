...
// Appending one task, rather than making the caller resend the whole day through
// writeTodo() and risk dropping what it did not know about.
export function addItem(file, { t, s = '', p: prio = '' }) {
  const text = String(t ?? '').trim();
  if (!text) throw new Error('texte de tâche requis');
  const cur = readTodo(file);
  if (cur.items.some(i => i.t.trim().toLowerCase() === text.toLowerCase())) return cur;
  return writeTodo(file, [...cur.items, { t: text, s: String(s).trim(), p: ['h', 'm'].includes(prio) ? prio : '', done: false }]);
}
